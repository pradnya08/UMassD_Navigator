    ...
    if (!isSigningIn) {
      ...
      doSignInWithEmailAndPassword(email, password).catch((err) => {
        if (err.code === "auth/invalid-credential") {
          setErrorMessage(
            "The email or password entered was incorrect, Please try again!"
          );
      ...
      <main className="w-full h-screen mt-[-96px] mx-auto flex-col flex self-center place-content-center place-items-center">
        <div className="w-96 text-white space-y-5 p-4 shadow-xl border rounded-xl">
          ...
          <p className="text-center text-sm">
            ...
            <Link
              to={"/register"}
              className="text-[#00df9a] hover:underline font-bold"
            >
              Sign up
            </Link>
          </p>
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-x-3 py-2.5 border rounded-lg text-sm font-medium  ${
              isSigningIn
                ? "cursor-not-allowed"
                : "hover:bg-gray-400 transition duration-300 active:bg-gray-100"
            }`}
          >
            <svg
              className="w-5 h-5"
